...
const AGENT_DIR = resolve(REPO_ROOT, ".agent");
...
describe("SOP Infrastructure", () => {
  describe("Directory Structure", () => {
    ...
    it("should have .agent/docs/sop/ with key SOP files", () => {
      const sopFiles = ["SOP.md", "tdd-workflow.md", "git-workflow.md"];
      for (const file of sopFiles) {
        expect(existsSync(resolve(AGENT_DIR, "docs/sop", file))).toBe(true);
      }
    });

    it("should have SOP_COMPLIANCE_CHECKLIST.md in docs root", () => {
      expect(existsSync(resolve(AGENT_DIR, "docs/SOP_COMPLIANCE_CHECKLIST.md"))).toBe(true);
    });
